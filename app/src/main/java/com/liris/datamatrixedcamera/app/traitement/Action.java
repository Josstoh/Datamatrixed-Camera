        ...
        for (int nl = FSize + 1; nl <= size - FSize - 1; nl++) {
            ...
            for (int nc = FSize + 1; nc <= size - FSize - 1; nc++) {
                ...
                for (int i = nc - FSize; i <= nc + FSize; i++) {
                    if (input.get(nl - FSize, i)[0] > max) {
                        max = input.get(nl - FSize, i)[0];
                    }
                }
...
//      Mat imRedressee =gridParameters(autocseuil,grayscaleMatrix);
    ...
    public Mat maxExtraction_(Mat input, int size, Mat grayscaleMatrix) {
        ...
        for (int i = 0; i < autocseuil.rows(); i++) {
            for (int j = 0; j < autocseuil.cols(); j++) {
                if (autocseuil.get(i, j)[0] == 255) {
                    nmbzero++;
                }
            }
        }
...
//    	for (int i=0;i<directionHorizontale.size();i++){
    ...
    public Mat profils(Mat imRedressee) {
        ...
        for (int i = 0; i < Profils10Colonne.length; i++) {
            System.out.println("signal10 " + Profils10Colonne[i]);
        }
        ...
        noZone = 16;
        if (signatureProfilColonne[noZone][5] == 1) {
            noZone++;
        }
        ...
        // Variables pour récupération des données
        int posMaxVerticale = 0, posMaxHorizontale =0;
        ...
        // Joss
        int it = 0;
        for(int i = n;i<posMaxHorizontale;i=i+2)
        {
            coordsHorizontales[it] = signatureProfil[i][3];
            it++;
        }
        coordsHorizontales[it] = signatureProfil[posMaxHorizontale][3];
        it++;
        ...
        // Joss
        for(int i = 0; i<coordsHorizontales.length;i++)
        {
            //Log.i("LIGNE HORIZONTALES","coords["+i+"] = "+coordsHorizontales[i]);
        }
        ...
        // Joss
        it = 0;
        for(int i = n;i<posMaxVerticale;i=i+2)
        {
            coordsVerticales[it] = signatureProfil[i][3];
            it++;
        }
        coordsVerticales[it] = signatureProfil[posMaxVerticale][3];
        it++;
        ...
        // Joss
        for(int i = 0; i<coordsVerticales.length;i++)
        {
            //Log.i("LIGNE VERTICALES","coords["+i+"] = "+coordsVerticales[i]);
        }
